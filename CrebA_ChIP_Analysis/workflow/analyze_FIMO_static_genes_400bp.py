...
static_bound_genes_df = pd.read_csv("../output/bound_genes_regulatory_regions_400bp/static_DE.csv", index_col = 0)
...
p_thresh = 0.0001
fimo_directory = "../output/bound_genes_regulatory_regions_400bp/static_genes/fly_factor_survey"
...
TF_df = pd.DataFrame(index = np.unique(TF_list), columns = static_bound_genes_df['feature'], data = 0)
...
for tmp_folder in folder_list: 
    ...
    fimo_genes = np.intersect1d(fimo_genes, static_bound_genes_df['feature'])
...
TF_df[TF_df > 1] = 1
TF_df.to_csv(os.path.join(out_path, 'static_flyfactorsurvey_hits.csv'))
...
prop_df.to_csv(os.path.join(out_path, 'static_flyfactorsurvey_hits_prop.csv'))
...
plt.barh(prop_df['TF_motifs'], prop_df['num_SPCGs'], color='#caba9c')
plt.rcParams['font.family'] = 'Arial'
# Add title and labels
plt.title('Top frequent TF motifs')
plt.xlabel('Number of SPCGs with TF motif')
plt.ylabel('Fly Factor Survey TFs')
plt.savefig(os.path.join(out_path, 'static_flyfactorssurvey_hits_prop.png'), dpi=300, bbox_inches='tight')

##### look at all the TF motifs combined #####
fimo_directory = "../output/bound_genes_regulatory_regions/static_genes/fly_factor_survey"
folder_list = os.listdir(fimo_directory)
folder_list = [x for x in folder_list if x != '.DS_Store']
folder_list = [x for x in folder_list if x != '.snakemake_timestamp']
folder_list = [x for x in folder_list if x != 'motif_database.txt']
...
TF_df.to_csv(os.path.join(out_path, 'static_flyfactorsurvey_combined_hits.csv'))

# summarize the results 
TF_df.sum(axis = 1).sort_values()
prop_df = pd.DataFrame(data = np.array([TF_df.index, TF_df.sum(axis = 1)]).T, columns = ['TF_motifs', 'num_SPCGs'])
prop_df = prop_df.sort_values('num_SPCGs')
prop_df.to_csv(os.path.join(out_path, 'static_flyfactorsurvey_combined_hits_prop.csv'))
