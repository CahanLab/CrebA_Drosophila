...
parser.add_argument('--min_num', type=int, help = 'minimum of cts')
...
# make DE_genes dataframe 
sc_down_genes_df = pd.DataFrame(index = np.unique(down_genes))
sc_up_genes_df = pd.DataFrame(index = np.unique(up_genes))

for tmp_key in cts_list: 
    sc_down_genes_df[tmp_key] = False
    sc_down_genes_df.loc[DE_genes_down_dict[tmp_key], tmp_key] = True

    sc_up_genes_df[tmp_key] = False
    sc_up_genes_df.loc[DE_genes_up_dict[tmp_key], tmp_key] = True

down_genes = np.array(sc_down_genes_df.apply(sum, axis = 1)[sc_down_genes_df.apply(sum, axis = 1) >= min_num].index)
up_genes = np.array(sc_up_genes_df.apply(sum, axis = 1)[sc_up_genes_df.apply(sum, axis = 1) >= min_num].index)
