...
# look at the CrebA hit peaks 
narrow_peaks = pd.read_csv(os.path.join("../output/intersect_region_CrebA/intersect_regions.narrowPeak"), sep="\t", header=None)
narrow_peaks = narrow_peaks.loc[narrow_peaks[2] - narrow_peaks[1] > 20, :]
...
for i in narrow_peaks.index:
    ...
    sub_promoter_tss['down_dist'] = narrow_peaks.loc[i, 1] - sub_promoter_tss['tss']
    sub_promoter_tss['up_dist'] = sub_promoter_tss['tss'] - narrow_peaks.loc[i, 2]
...
if os.path.isdir("../output/match_nearest_intersect_250_peaks/") == False:
    os.makedirs("../output/match_nearest_intersect_250_peaks/")
narrow_peaks.to_csv("../output/match_nearest_intersect_250_peaks/match_nearest_genes.csv")
