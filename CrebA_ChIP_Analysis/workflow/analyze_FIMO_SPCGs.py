...
out_path = '../output/SPCG_promoters'

##### look at the CrebA motifs #####
fimo_directory = "../output/SPCG_promoters/CrebA_motifs"
...
crebA_df = pd.DataFrame(index = folder_list, columns = spcg_list, data = 0)
...
for tmp_folder in folder_list: 
    ...
    crebA_df.loc[tmp_folder, fimo_spcgs] = crebA_df.loc[tmp_folder, fimo_spcgs] + 1

crebA_df.to_csv(os.path.join(out_path, 'crebA_hits_0.0001.csv'))

##### look at all the other TF motifs ######
fimo_directory = "../output/SPCG_promoters/fly_factor_survey"
folder_list = os.listdir(fimo_directory)
folder_list = [x for x in folder_list if x != '.DS_Store']
folder_list = [x for x in folder_list if x != 'motif_database.txt']
folder_list = [x for x in folder_list if 'untrimmed' not in x]
...
TF_df[TF_df > 1] = 1
TF_df.to_csv(os.path.join(out_path, 'flyfactorsurvey_hits_0.0001.csv'))
...
prop_df.to_csv(os.path.join(out_path, 'flyfactorsurvey_hits_0.0001_prop.csv'))
...
TF_df.to_csv(os.path.join(out_path, 'flyfactorsurvey_combined_hits_0.0001.csv'))

# summarize the results 
TF_df.sum(axis = 1).sort_values()
prop_df = pd.DataFrame(data = np.array([TF_df.index, TF_df.sum(axis = 1)]).T, columns = ['TF_motifs', 'num_SPCGs'])
prop_df = prop_df.sort_values('num_SPCGs')
prop_df.to_csv(os.path.join(out_path, 'flyfactorsurvey_combined_hits_0.0001_prop.csv'))
