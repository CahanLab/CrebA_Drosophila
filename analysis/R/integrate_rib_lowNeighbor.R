...
SAMPLETYPE = "rib"
TARGET_dir = file.path("results", ANALYSIS_VERSION, "rib_integrated_lowK")
...
num_pc = 90 # this is hard coded 
object %<>% RunPCA(features = VariableFeatures(object = object), npcs = num_pc)
object %<>% FindNeighbors(dims = 1:num_pc) # to really break down the clusters. Probably will need to do the same for the wild types
...
object %<>% FindClusters(resolution = 1.5) # more reasonable resolution 

saveRDS(object, file = file.path(TARGET_dir, 'object.rds'))

# perform clustering
object = readRDS(file = file.path(TARGET_dir, 'object.rds'))

###############
# to see if I can pinpoint out the salivary glands cells 
# the below was just done for sanity check to see if we can pinpoint a cluster 
FeaturePlot(object, features = 'CG13159')
FeaturePlot(object, features = 'pip')
#FeaturePlot(object, features = 'nur')

VlnPlot(object, features = 'pip', pt.size = 0, group.by = 'RNA_snn_res.6')
VlnPlot(object, features = 'CG13159', pt.size = 0, group.by = 'RNA_snn_res.6')
VlnPlot(object, features = 'trh', pt.size = 0)

object@meta.data$target_cluster = object@meta.data$RNA_snn_res.6 == 82 #51
...
VlnPlot(object, 'CG13159')
saveRDS(object, file = file.path(TARGET_dir, 'BDGP_automated_annotation_object.rds'))

# manually assign the tiny cluster with 
object@meta.data[object@meta.data$RNA_snn_res.6 == 82, 'Integrated_tentativeCellType'] = 'salivary gland'

DimPlot(object, group.by = 'Integrated_tentativeCellType', label = T)
saveRDS(object, file = file.path(TARGET_dir, 'BDGP_automated_annotation_object.rds'))
