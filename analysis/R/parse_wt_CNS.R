...
SAMPLETYPE = "wt"
TARGET_dir = file.path("results", ANALYSIS_VERSION, "parse_wt_CNS")
dir.create(TARGET_dir)

object = readRDS(file.path("results", ANALYSIS_VERSION, 'manual_annotation_wt13/manual_celltype_object2.rds'))

sub_object = subset(object, cells = rownames(object@meta.data)[grepl("CNS", object@meta.data$manual_celltypes)])
...
ggsave(file.path(TARGET_dir, "log10_nCount_RNA.png" ), width = 8, height = 6)
...
cytoTrace_meta = read.csv(file.path("results", ANALYSIS_VERSION, "wt13_cytoTrace/CNS/", "cytotraced_sample_tab.csv"), row.names = 1)
