...
SAMPLETYPE = "rib"
TARGET_dir = file.path("results", ANALYSIS_VERSION, "rib_integrated")
...
###############################################
# really extract out the salivary gland via clustering 
set.seed(1)
object = readRDS(file.path(TARGET_dir, 'BDGP_automated_annotation_object.rds'))
object %<>% FindClusters(resolution = 9)
VlnPlot(object, features = 'pip', pt.size = 0)

object@meta.data$target_cluster = object@meta.data$seurat_clusters == 54 #51
