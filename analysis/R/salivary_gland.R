...
TARGET_dir = file.path("results", ANALYSIS_VERSION, "salivary_gland")
dir.create(TARGET_dir)

wt_object = readRDS(file.path("results", ANALYSIS_VERSION, "wt13_integrated", "BDGP_automated_annotation_object.rds"))
rib_object = readRDS(file.path("results", ANALYSIS_VERSION, "rib_epidermis", "rib_epidermis_finer_object.rds"))

ts_wt_obj = subset(wt_object, subset = Integrated_tentativeCellType == 'salivary gland') # subset out the salivary gland
ts_wt_obj@meta.data$experimental_condition = 'Wt'

ts_rib_obj = subset(rib_object, subset = finer_annotation == 'salivary gland') # subset out the salivary gland
...
write.csv(markers, file = file.path(TARGET_dir, 'DE_genes_wt_rib_sg_wilcox.csv'))
...
exp_dat = object@assays$RNA@data[rownames(markers), ]
...
for(temp_samp in my_sample_col$sample) {
