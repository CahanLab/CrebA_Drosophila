...
SAMPLETYPE = "rib"
TARGET_dir = file.path("results", ANALYSIS_VERSION, "rib_epidermis")
dir.create(TARGET_dir)

rib_object = readRDS(file.path("results", ANALYSIS_VERSION, "rib_integrated", 'BDGP_automated_annotation_object.rds'))
rib_object = subset(rib_object, subset = (Integrated_tentativeCellType == "dorsal epidermis" | Integrated_tentativeCellType == "ventral epidermis"))

raw_object = Seurat::CreateSeuratObject(rib_object@assays$RNA@counts, project = 'rib_epidermis')
raw_object@meta.data$batch = rib_object@meta.data$batch
raw_object@meta.data$broad_ct = rib_object@meta.data$Integrated_tentativeCellType
...
object@meta.data$target_cluster = object@meta.data$seurat_clusters == 26 #51
#DimPlot(object)
DimPlot(object,group.by =  'target_cluster')

object@meta.data$finer_annotation = object@meta.data$broad_ct
object@meta.data[object@meta.data$seurat_clusters == 26, 'finer_annotation'] = 'salivary gland'

DimPlot(object, group.by = 'finer_annotation')
ggsave(filename = file.path(TARGET_dir, 'finner_epidermis.png'), width = 8, height = 6)

saveRDS(object, file = file.path(TARGET_dir, 'rib_epidermis_finer_object.rds'))
