...
object = readRDS(file.path("results", ANALYSIS_VERSION, "crebA_integrated/BDGP_automated_annotation_object.rds"))
...
for(cluster in unique(object@meta.data$seurat_clusters)) {
  if(cluster == '37') {
    object@meta.data[object@meta.data$seurat_clusters == cluster, 'manual_celltypes'] = 'Unknown'
  } else { 
