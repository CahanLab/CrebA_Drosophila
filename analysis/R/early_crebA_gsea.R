TARGET_dir = file.path("results", ANALYSIS_VERSION, "early_crebA_gsea")
dir.create(TARGET_dir)

object = readRDS(file.path("results", ANALYSIS_VERSION, "manual_annotation_crebA_early", 'manual_celltype_object.rds'))
...
withr::with_dir(TARGET_dir, {
  for(celltype in unique(object$manual_celltypes)) {
    if(stringr::str_replace_all(celltype, "/", "-") %in% list.dirs(".", recursive = FALSE, full.names = FALSE)) {
      ...
      write.csv(fgseaRes, file = file.path(file.path(stringr::str_replace_all(celltype, "/", "-")), 'gsea_results_early_crebA.csv'))
...
for(my_folder in all_folders) {
  print(my_folder)
  gsea_results = read.csv(file.path(my_folder, 'gsea_results_early_crebA.csv'), row.names = 1)
