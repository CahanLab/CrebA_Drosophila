...
SAMPLETYPE = "wt"
TARGET_dir = file.path("results", ANALYSIS_VERSION, "parse_wt_epidermis")
dir.create(TARGET_dir)

object = readRDS(file.path("results", ANALYSIS_VERSION, 'manual_annotation_wt13/manual_celltype_object2.rds'))

sub_object = subset(object, cells = rownames(object@meta.data)[grepl("Epidermis", object@meta.data$manual_celltypes)])
...
sub_object %<>% FindClusters(resolution = 0.3)
...
FeaturePlot(sub_object, 'Epipharynx_ModuleScore1')
FeaturePlot(sub_object, 'Ventral_Epidermis_ModuleScore1')
FeaturePlot(sub_object, 'Dorsal_Epidermis_ModuleScore1')
FeaturePlot(sub_object, 'Esophagus_ModuleScore1')
FeaturePlot(sub_object, 'Hypopharynx_ModuleScore1')
...
ggsave(file.path(TARGET_dir, "Finer_cellTypes.png" ), width = 8, height = 6)
...
FeaturePlot(sub_object, features = 'nFeature_RNA')
ggsave(file.path(TARGET_dir, "nFeature_RNA.png" ), width = 20, height = 7 )

DimPlot(sub_object, group.by = 'Phase')
ggsave(file.path(TARGET_dir, "Phase.png" ), width = 8, height = 6 )

cytoTrace_meta = read.csv(file.path("results", ANALYSIS_VERSION, "wt13_cytoTrace/Epidermis/", "cytotraced_sample_tab.csv"), row.names = 1)
