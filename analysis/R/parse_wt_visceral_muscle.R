...
SAMPLETYPE = "wt"
TARGET_dir = file.path("results", ANALYSIS_VERSION, "parse_wt_visceral_muscle")
dir.create(TARGET_dir)

object = readRDS(file.path("results", ANALYSIS_VERSION, 'manual_annotation_wt13/manual_celltype_object3.rds'))

sub_object = subset(object, cells = rownames(object@meta.data)[grepl("Visceral", object@meta.data$manual_celltypes) |
                                                                 grepl("Hindgut Muscle", object@meta.data$manual_celltypes) |
                                                                 grepl("Pharyngeal Muscle", object@meta.data$manual_celltypes)])
...
sub_object %<>% FindClusters(resolution = 0.4)
pdf(file = file.path(TARGET_dir, "harmony_integration.pdf"))
DimPlot(sub_object, group.by = "batch")
dev.off()

DimPlot(sub_object, group.by = 'manual_celltypes')
ggsave(file.path(TARGET_dir, "old_label_integration.png" ), width = 10, height = 6)
...
library(CytoTRACE)
sub_object = readRDS(file.path(TARGET_dir, 'finer_annotation_object.rds'))

raw_exp = sub_object@assays$RNA@counts
batch1_exp = raw_exp[, rownames(sub_object@meta.data[sub_object@meta.data$batch == 'rep_1', ])]
batch3_exp = raw_exp[, rownames(sub_object@meta.data[sub_object@meta.data$batch == 'rep_3', ])]
datasets <- list(as.matrix(batch1_exp), as.matrix(batch3_exp))
results <- iCytoTRACE(datasets)
saveRDS(results, file = file.path(TARGET_dir, "cytoTrace_results.rds"))

sub_object@meta.data$cytoTrace_scores = results$CytoTRACE[colnames(raw_exp)]
FeaturePlot(sub_object, features = 'cytoTrace_scores')
VlnPlot(sub_object, features = 'cytoTrace_scores')

FeaturePlot(sub_object, 'cytoTrace_scores')
