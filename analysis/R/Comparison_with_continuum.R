...
TARGET_dir = file.path("results", ANALYSIS_VERSION, "compare_with_continuum")
dir.create(TARGET_dir)

seurat_data = readRDS("accessory_data/continuum_drosophila_embryonic_development/processed_data/continuum_exploration/14-16_celltyped.rds")
...
p<-ggplot(data=plot_df, aes(x=reorder(cell_types, proportion), y=proportion, fill = cell_types)) +
  geom_bar(stat="identity") + theme_bw() + coord_flip() + 
  ylab("Total Cell Proportion") + 
  xlab("Cell Types") + 
  ggtitle("stage 14-16: Continuum")
ggsave(filename = file.path(TARGET_dir, 'Continuum_cell_stg14-16_proportion.png'), plot = p, width = 14, height = 6)
...
p = ggplot(combination_df, aes(our_ct, other_ct, fill= spearman_correlation)) + 
  ...
  ggtitle("correlation similarity between our cell type and Continuum's cell type")
ggsave(filename = file.path(TARGET_dir, 'Continuum_cell_correlation.png'), plot = p, width = 14, height = 6)
