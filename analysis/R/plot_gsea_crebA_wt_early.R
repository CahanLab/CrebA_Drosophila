...
##### make the wildtype plot - SG, PL, AM, FB #####
...
ct_interest = c('Trachea', 'Glia', 'Hindgut')
color_palette = readRDS(file.path('results', ANALYSIS_VERSION, 'ct_color_palettes/ct_color_palette.rds'))
...
p <- ggplot(data = sub_wt_df, aes(y = reorder(pathway, logpval), x = logpval, fill = celltype)) +
  ...
  theme(strip.text.x = element_blank(), axis.text.x=element_text(angle=0, vjust = 1, hjust=1)) +
  ggtitle('Geneset enrichment in stage 10-12 wildtype embryos')  
ggsave(filename = file.path(TARGET_dir, 'wildtype_selected_ct2.png'), width = 11, height = 10)
...
color_palette = readRDS(file.path('results', ANALYSIS_VERSION, 'ct_color_palettes/ct_color_palette.rds'))
sub_wt_df = wt_df[wt_df$celltype %in% ct_interest, ]
sub_wt_df <- droplevels(sub_wt_df)
sub_wt_df[sub_wt_df$celltype == 'Circular Visceral Mesoderm', 'celltype'] = 'Circ. Vis. Meso.'
sub_wt_df$celltype = factor(x = sub_wt_df$celltype, levels = c(ct_interest, 'Circ. Vis. Meso.'))
...
p <- ggplot(data = sub_wt_df, aes(y = reorder(pathway, logpval), x = logpval, fill = celltype)) +
  ...
  theme(strip.text.x = element_blank(), axis.text.x=element_text(angle=0, vjust = 1, hjust=1)) +
  ggtitle('Geneset enrichment in stage 10-12 wildtype embryos')  
ggsave(filename = file.path(TARGET_dir, 'wildtype_selected_ct3.png'), width = 11.2, height = 12)

sub_mut_df = mut_df[mut_df$celltype %in% ct_interest, ]

sub_mut_df <- droplevels(sub_mut_df)
sub_mut_df[sub_mut_df$celltype == 'Circular Visceral Mesoderm', 'celltype'] = 'Circ. Vis. Meso.'
sub_mut_df$celltype = factor(x = sub_mut_df$celltype, levels = c(ct_interest, 'Circ. Vis. Meso.'))
...
p <- ggplot(data = sub_mut_df, aes(y = reorder(pathway, logpval), x = logpval, fill = celltype)) +
  ...
  theme(strip.text.x = element_blank(), axis.text.x=element_text(angle=0, vjust = 1, hjust=1)) +
  ggtitle('Geneset enrichment in stage 10-12 mutant embryos')  
ggsave(filename = file.path(TARGET_dir, 'mutant_selected_ct3.png'), width = 11.2, height = 12)
